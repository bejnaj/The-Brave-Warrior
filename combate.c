...
void printb(const char *fmt, ...) {
    ...
    for (int i = 0; buffer[i] != '\0'; i++) {
        putchar(buffer[i]);
        fflush(stdout);
        Sleep(20); // Tiempo de espera entre caracteres en milisegundos (ajusta a tu gusto)
    }
}
...
int main(){
    ...
    Item *itemPrueba;
    itemPrueba->nombre = strdup("Capa de Protección");
    itemPrueba->descripcion = strdup("Capa encantada, +25 defensa");
    itemPrueba->tipoCons = noConsumible;
    itemPrueba->tipoEquip = ARMADURA;
