...
void *list_popCurrent(List *L) { // Modificada para devovler como nuevo current el nodo anterior al borrado
  if (L == NULL || L->current == NULL) {
...
void listDataClean(List *L) {
  ...
  while (current != NULL) {
    next = current->next;
    free(current -> data); // Limpiar tambien el dato
    free(current);
    current = next;
  }
  L->head = NULL;
